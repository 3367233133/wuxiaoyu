...
import com.niuma.rainapi.common.RedisTokenBucket;
import com.niuma.rainapi.model.vo.UserDevKeyVO;
import com.niuma.rainapicommon.constant.UserConstant;
import com.niuma.rainapi.mapper.UserMapper;
import com.niuma.rainapi.model.dto.user.UserRegisterRequest;
import com.niuma.rainapi.service.UserService;
import com.niuma.rainapi.utils.SmsUtils;
...
import com.niuma.rainapicommon.utils.AuthPhoneNumberUtil;
...
import static com.niuma.rainapicommon.constant.CookieConstant.COOKIE_KEY;
import static com.niuma.rainapicommon.constant.CookieConstant.COOKIE_USER_KEY;
import static com.niuma.rainapicommon.constant.UserConstant.*;
...
public class UserServiceImpl extends ServiceImpl<UserMapper, User>
    ...
    // todo 先获取值，然后校验参数，图形验证码和手机验证码都保存在redis缓存中，将用户的密码加上一个盐值进行md5加密
    // todo 并且分配ak,sk进行接口校验
    @Override
    public Long userRegister(UserRegisterRequest userRegisterRequest, HttpServletRequest request) {
        ...
        //防止用户多次登录，并发情况下会出现同时校验数据库是否存在数据，若都判断出没有该用户，那么就会出现同时同一个用户插入多条数据
    ...
    // todo 先校验手机验证码是否和缓存中的研验证一致，然后根据过滤条件查询这个手机号是否注册过，
    @Override
    ...
    // todo 先通过hutool的验证码工具类生成随即图以及四个随机数字，然后写入到输出流中，并且将图形验证码保存到redis中，方便登陆和注册的时候做校验
    @Override
    ...
    // todo 服务端这边生成手机短信验证码，然后发给腾讯的sms，让腾讯服务发送给用户，但是为了异步操作和解耦操作，先把短信发送给消息队列，然后让消息队列的消费端发送给腾讯
    @Override
